...
export async function saveMealLibraryItem(formData: FormData) {
  const supabase = requireAdminClient();
  const id = value(formData, "id");
  const nameEn = value(formData, "name_en");
  const nameNb = value(formData, "name_nb");
  ...
  const payload = {
    slug: value(formData, "slug") || slugify(nameEn),
    category: value(formData, "category") || "breakfast",
    kcal: intValue(formData, "kcal", 0),
    protein_g: numberValue(formData, "protein_g") ?? 0,
    carbs_g: numberValue(formData, "carbs_g") ?? 0,
    fats_g: numberValue(formData, "fats_g") ?? 0,
    name_translations: translations(nameEn, nameNb),
    note_translations: translations(
      value(formData, "note_en"),
      value(formData, "note_nb"),
    ),
    is_active: formData.get("is_active") === "on",
  ...
  const result = id
    ? await supabase.from("meal_library").update(payload).eq("id", id)
    : await supabase.from("meal_library").insert(payload);

  if (result.error) throw new Error(result.error.message);

  revalidatePath("/nutrition/library");
}

// Soft delete — keeps meal_logs.meal_library_id references intact.
// Restore by editing the row and re-checking "Active".
export async function deleteMealLibraryItem(formData: FormData) {
  const supabase = requireAdminClient();
  const id = value(formData, "id");
...
// ---------------------------------------------------------------------------
// Nutrition — meal programs (auto-scaffolds 3 phases × 7 days on insert)
// ---------------------------------------------------------------------------

const MEAL_PHASE_KEYS = ["hypertrophy", "strength", "peak"] as const;

export async function saveMealProgram(formData: FormData) {
  const supabase = requireAdminClient();
  const id = value(formData, "id");
  const titleEn = value(formData, "title_en");
  const titleNb = value(formData, "title_nb");
  const descEn = value(formData, "description_en");
  const descNb = value(formData, "description_nb");
  ...
  const payload = {
    title_translations: translations(titleEn, titleNb),
    description_translations: translations(descEn, descNb),
    duration_days: intValue(formData, "duration_days", 84),
    is_active: formData.get("is_active") === "on",
    updated_at: new Date().toISOString(),
  };
  ...
  if (id) {
    ...
    const phaseInserts = MEAL_PHASE_KEYS.map((key, idx) => ({
      meal_program_id: data.id,
      phase_key: key,
      sort_order: idx,
      week_count: 4,
    }));
    ...
    const dayInserts = (phases ?? []).flatMap((phase) =>
      [1, 2, 3, 4, 5, 6, 7].map((dow) => ({
        meal_program_phase_id: phase.id,
        day_of_week: dow,
      })),
    );
  ...
  revalidatePath("/nutrition/programs");
}

export async function deleteMealProgram(formData: FormData) {
  const supabase = requireAdminClient();
  const id = value(formData, "id");
...
export async function saveMealProgramPhase(formData: FormData) {
  const supabase = requireAdminClient();
  const id = value(formData, "id");
  const programId = value(formData, "program_id");
  ...
  const { error } = await supabase
    .from("meal_program_phases")
    .update({
      week_count: intValue(formData, "week_count", 4),
      kcal_target: numberValue(formData, "kcal_target"),
      protein_g_target: numberValue(formData, "protein_g_target"),
      carbs_g_target: numberValue(formData, "carbs_g_target"),
      fats_g_target: numberValue(formData, "fats_g_target"),
      updated_at: new Date().toISOString(),
    })
    .eq("id", id);
  if (error) throw new Error(error.message);

  revalidatePath(`/nutrition/programs/${programId}`);
}

export async function addMealProgramPhaseDayItem(formData: FormData) {
  const supabase = requireAdminClient();
  const phaseDayId = value(formData, "phase_day_id");
  const libraryId = value(formData, "library_id");
  const programId = value(formData, "program_id");

  if (!phaseDayId || !libraryId) {
    throw new Error("Missing meal or phase day.");
  }

  const { data: existing } = await supabase
    .from("meal_program_phase_day_items")
    .select("sort_order")
    .eq("meal_program_phase_day_id", phaseDayId)
  ...
  const { error } = await supabase
    .from("meal_program_phase_day_items")
    .insert({
      meal_program_phase_day_id: phaseDayId,
      meal_library_id: libraryId,
      sort_order: nextSort,
    });
  if (error) throw new Error(error.message);
...
export async function removeMealProgramPhaseDayItem(formData: FormData) {
  const supabase = requireAdminClient();
  const id = value(formData, "id");
  const programId = value(formData, "program_id");

...
- `doc/PAUSE_WORKOUT.md` — Pause / Resume / End workout flow (locked with client 2026-07-11). Pause = save & resume to the exact set; End = finish now + remaining marked `skipped`; Resume vs Start Again by pending-exercise state; leave bottom sheet = Pause + End (no Keep Going); optimistic skip writes; points/PR awarded once per exercise. Read before touching the active-session leave flow, session timer, or Resume/Start-Again logic.

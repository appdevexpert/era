...
- `doc/EXERCISE_DEMO_VIDEOS.md` — Admin-managed male/female demo clips on the Workout Log tile. Locked gender-pick and cross-gender fallback rules, the loop toggle's tap-to-play behaviour, why the storage bucket is public instead of signed, and why uploads bypass Server Actions (1 MB body cap). Read before touching `exerciseMedia.ts`, `ExerciseAnimationCard`, or the admin upload field.
- `doc/EXERCISE_INFO_SHEET.md` — Read-only exercise info bottom sheet (tap any planned row on the Exercise List). Covers the payload-from-the-row rule, the `description_translations` "Form detail" wiring, clip coverage + query, and the bulk Zoho→Supabase clip import. **Read the Gotchas before touching any bottom sheet:** `BottomSheetModalProvider` sits outside `NavigationContainer`, so `useIsFocused()` throws inside sheet content; plus why `ExerciseAnimationCard` must never get horizontal margin.
- `doc/EXERCISE_REORDER.md` — Drag-and-drop exercise reordering on the Exercise List screen. UX rules (handle only shows active + not-started + 2+ exercises), WHY reorder is blocked once the workout starts (session order frozen → list/session desync, resume pointer breaks), the deferred "reorder during resume" feature, and the full architecture (table, Redux overlay, sync queue, mappers). Read before touching the reorder handle or the order overlay.
- `doc/MODALITY_AND_KIND.md` — Admin-panel client demo briefing (Hinglish). Distinguishes exercise-level `Modality` (drives mobile log UI: strength/cardio/core/mobility) from set-level `Kind` (drives PR eligibility, points bonuses, Smart Weight Engine: working/top_set/backoff/core/cardio). Documents the Modality→Kind auto-filter contract, why warmup/drop_set/amrap were removed from the Kind picker, and includes a full Rami-facing demo script with Q&A. Read before demoing the Program Builder or explaining `planned_set_kind` to the client.
- `doc/AUTH_EMAIL_TEMPLATES.md` — Supabase auth email templates (confirm signup / reset password / OTP). Where the files live, why the Dashboard is the only deploy path, the "write to the person, not about their user" copy rule, and the English-only gap.
...
- `doc/IOS_SCENE_LIFECYCLE.md` — Why `plugins/withIosSceneLifecycle.js` exists: Xcode 27 / iOS 27 SDK refuses to launch apps without a `UIApplicationSceneManifest` + `UISceneDelegate`, and Expo SDK 54's template hasn't migrated. Covers the AppDelegate → SceneDelegate split, the deep links that must be hand-forwarded once a scene delegate exists, and the ordering constraint with `@react-native-firebase/app`. Read before touching the iOS AppDelegate, deep linking, or the splash screen.

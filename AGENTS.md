...
Each sub-project has its own `node_modules`, `package.json`, and build tooling. Run `npm install` inside the respective directory. See `mobile/AGENTS.md` for detailed mobile architecture and `web/AGENTS.md` for Next.js 16 caveats.

## Documentation Rule

All new feature/reference markdown docs (schema walkthroughs, feature specs, integration notes) MUST be created inside `mobile/doc/`. Do NOT create new `*.md` docs at the `mobile/` root or repo root. The only allowed root-level `.md` files are `AGENTS.md`, `Codex.local.md`, and `README.md`. When you add a new doc, also link it in `mobile/AGENTS.md` under the Feature Docs list so future sessions can discover it.
